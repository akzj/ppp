...
// TestSourceS3Type verifies the S3/OSS types yield an S3 source.
func TestSourceS3Type(t *testing.T) {
	for _, typ := range []pppv1.Source_Type{pppv1.Source_OSS, pppv1.Source_S3} {
		if _, err := NewSource(typ); err != nil {
			t.Fatalf("NewSource(%v) = %v, want an s3 source", typ, err)
		}
	}
}

// TestSourceS3Range verifies fetching a piece from an S3-compatible endpoint
// (path-style GET /bucket/key with a Range header).
func TestSourceS3Range(t *testing.T) {
	content := []byte("0123456789abcdefghij") // 20 bytes
	srv := httptest.NewServer(http.HandlerFunc(func(w http.ResponseWriter, r *http.Request) {
		// Path-style: /bucket/key.
		if r.URL.Path != "/bucket/a.bin" {
			http.NotFound(w, r)
			return
		}
		var start, end int
	...
	s, err := NewSource(pppv1.Source_S3)
	if err != nil {
		t.Fatalf("NewSource: %v", err)
	}
	src := &pppv1.Source{
		Type: pppv1.Source_S3, Urls: []string{srv.URL}, Bucket: "bucket", Key: "a.bin",
	}
	p0, err := s.FetchPiece(context.Background(), src, "t1", "f", 0, int64(len(content)), 8)
	if err != nil {
		t.Fatalf("FetchPiece(0): %v", err)
...
// TestSourceS3MirrorFailover verifies a failing endpoint falls back to the
// next one.
func TestSourceS3MirrorFailover(t *testing.T) {
	content := []byte("abcdef")
	bad := httptest.NewServer(http.HandlerFunc(func(w http.ResponseWriter, r *http.Request) {
		w.WriteHeader(http.StatusInternalServerError)
	}))
	...
	src := &pppv1.Source{
		Type: pppv1.Source_S3, Urls: []string{bad.URL, good.URL}, Bucket: "bucket", Key: "a.bin",
	}
	data, err := s.FetchPiece(context.Background(), src, "t1", "f", 0, int64(len(content)), 8)
	if err != nil {
		t.Fatalf("FetchPiece: %v", err)
